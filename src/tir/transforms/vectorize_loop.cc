...
class LocalPadder : public StmtExprMutator {
 private:
  ...
  Stmt VisitStmt_(const StoreNode* op) final {
    ...
    CHECK(op->value.dtype().is_float())
        << "The current implementation assumes that the data type is FP32";
    return Store(op->buffer_var,
                 Select(merged_predicate, op->value, PrimExpr(0.f)),
                 op->index, op->predicate);
  }
  PrimExpr VisitExpr_(const VarNode* op) final {
    if (substitute_mode_ &&
        (op->name_hint == "threadIdx.x" ||
         StrEndsWith(op->name_hint, ".fused.outer.outer"))) {
      return Integer(0);
    }
 ...
 public:
  ...
  bool StrEndsWith(const String& a, const String& b) const {
    if (b.size() > a.size()) return false;
    return std::equal(a.c_str() + a.size() - b.size(),
                      a.c_str() + a.size(),
                      b.c_str());
  }
 private:
